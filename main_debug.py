...
async def procesar_csv(file: UploadFile = File(...)):
    try:
        ...
        print("🔀 Realizando merge con columna:", col_codigo)

        # ✅ Renombrar columna conflictiva si colisiona con shapefile
        ...
        print("🌍 Exportando KML...")
        gdf_out.to_crs("EPSG:4326").to_file(output_kml, driver="KML")

        print("🗜️ Comprimir archivos...")
        zip_path = f"{output_dir}.zip"
        with zipfile.ZipFile(zip_path, "w") as zipf:
            for fname in os.listdir(output_dir):
                print("➕ Agregando al ZIP:", fname)
                zipf.write(os.path.join(output_dir, fname), arcname=fname)

        print("✅ ZIP generado correctamente con", len(gdf_out), "registros")
