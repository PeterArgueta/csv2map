...
export function ConvertirFormatos() {
  ...
  const chooseFile = (selected) => {
    ...
    if (!detected) {
      setFile(null);
      setInputFormat('');
      setError('Formato no compatible. Usa GeoJSON, ZIP de Shapefile, KML, GeoPackage o CSV.');
      return;
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      let detail = 'No fue posible convertir el archivo.';
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="mb-7">
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-indigo-600">Conversor GIS</p>
        <h2 className="mt-2 text-3xl font-bold tracking-tight">Convierte formatos geográficos</h2>
        <p className="mt-2 max-w-3xl leading-7 text-slate-600">
          Convierte una capa entre GeoJSON, Shapefile, KML, GeoPackage y CSV sin depender de códigos territoriales.
        </p>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="font-bold">1. Carga tu capa</h3>
              <p className="mt-1 text-sm text-slate-500">El formato de entrada se detecta automáticamente.</p>
            </div>
            {inputFormat && (
          ...
          >
            ...
            <p className="mt-3 font-semibold text-slate-700">{file ? file.name : 'Selecciona o arrastra una capa'}</p>
            <p className="mt-1 text-xs text-slate-500">GeoJSON · SHP (.zip) · KML · GPKG · CSV · máximo 10 MB</p>
            <input
              ref={inputRef}
              type="file"
          ...
          {inputFormat === 'csv' && (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50/60 p-4">
              <p className="text-sm font-bold text-slate-800">Coordenadas del CSV</p>
              <p className="mt-1 text-xs leading-5 text-slate-600">
                El CSV debe representar puntos. Indica las columnas que contienen coordenadas en WGS84.
              </p>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label>
                  <span className="field-label">Columna de latitud</span>
                  <input value={latColumn} onChange={(event) => setLatColumn(event.target.value)} className="field-control" placeholder="latitud" />
                </label>
                <label>
                  <span className="field-label">Columna de longitud</span>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h3 className="font-bold">2. Elige el formato de salida</h3>
          <p className="mt-1 text-sm text-slate-500">La geometría y los atributos se conservan durante la conversión.</p>
              ...
              return (
                <label key={option.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${selected ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200 hover:border-slate-300'} ${same ? 'opacity-50' : ''}`}>
                  ...
                  <span>
                    ...
                    <span className="block text-xs text-slate-500">{same ? 'Es el formato de entrada' : option.note}</span>
                  </span>
                </label>
              );
            })}
          ...
          >
            {converting ? 'Convirtiendo…' : `Convertir a ${outputInfo?.label || ''}`}
          </button>

          <div className="mt-4 rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">
            <strong className="text-slate-700">Nota:</strong> Shapefile se descarga como ZIP porque necesita varios archivos. Al exportar a CSV se agrega una columna <code>geometry_wkt</code>; para puntos también se incluyen latitud y longitud.
          </div>
        </section>
      </div>
